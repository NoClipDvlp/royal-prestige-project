...
-- Fuente: migraciones + RLS del repo. Orden: 0000 → policies → 0001 → 0002 → 0003 → 0004 → 0005 → 0006 → 0008 → 0009 → 0010 → 0011 → 0012 → 0013 → 0014 → 0015 → 0016 → 0017 → 0018 → 0019 → 0020.
...
-- =====================================================================
-- >>> db/migrations/0016_set_task_status.sql
-- =====================================================================
...
-- =====================================================================
-- >>> db/migrations/0018_ms_module.sql
-- =====================================================================
-- ============================================================================
...
alter table public.users add column if not exists ms_mailing_enabled boolean not null default false;
...
create table if not exists public.ms_recipients (
  id            uuid primary key default gen_random_uuid(),
...
create table if not exists public.ms_templates (
  id            uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null references public.users(id) on delete cascade,
  name          text not null,
...
create table if not exists public.ms_sends (
  id                  uuid primary key default gen_random_uuid(),
...
create table if not exists public.ms_suppressions (
  id            uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null references public.users(id) on delete cascade,
  email         text not null,
...
create unique index if not exists uq_ms_recipients_dataset_email on public.ms_recipients (dataset_id, lower(email)); -- dedupe
create unique index if not exists uq_ms_sends_campaign_email on public.ms_sends (campaign_id, lower(email));   -- idempotencia
create unique index if not exists uq_ms_suppr_owner_email   on public.ms_suppressions (owner_user_id, lower(email));
create index if not exists idx_ms_sends_unsub_token on public.ms_sends (unsub_token);                          -- baja por token

create index if not exists idx_ms_datasets_owner    on public.ms_datasets (owner_user_id);
create index if not exists idx_ms_recipients_owner  on public.ms_recipients (owner_user_id);
create index if not exists idx_ms_recipients_dataset on public.ms_recipients (dataset_id);
create index if not exists idx_ms_templates_owner   on public.ms_templates (owner_user_id);
create index if not exists idx_ms_campaigns_owner   on public.ms_campaigns (owner_user_id);
create index if not exists idx_ms_sends_owner       on public.ms_sends (owner_user_id);
create index if not exists idx_ms_sends_campaign_status on public.ms_sends (campaign_id, status);              -- reanudación
create index if not exists idx_ms_suppr_owner       on public.ms_suppressions (owner_user_id);

-- ── 5) updated_at (reusa el helper de 0000) ───────────────────────────────────
create or replace trigger trg_ms_datasets_updated  before update on public.ms_datasets  for each row execute function public.set_updated_at();
create or replace trigger trg_ms_templates_updated before update on public.ms_templates for each row execute function public.set_updated_at();
create or replace trigger trg_ms_campaigns_updated before update on public.ms_campaigns for each row execute function public.set_updated_at();
create or replace trigger trg_ms_sends_updated     before update on public.ms_sends     for each row execute function public.set_updated_at();
...
drop policy if exists ms_datasets_self on public.ms_datasets;
...
drop policy if exists ms_recipients_self on public.ms_recipients;
create policy ms_recipients_self on public.ms_recipients for all
  using (owner_user_id = (select auth.uid()) and public.ms_enabled())
  with check (owner_user_id = (select auth.uid()) and public.ms_enabled());
drop policy if exists ms_templates_self on public.ms_templates;
create policy ms_templates_self on public.ms_templates  for all
  using (owner_user_id = (select auth.uid()) and public.ms_enabled())
  with check (owner_user_id = (select auth.uid()) and public.ms_enabled());
drop policy if exists ms_campaigns_self on public.ms_campaigns;
create policy ms_campaigns_self on public.ms_campaigns  for all
  using (owner_user_id = (select auth.uid()) and public.ms_enabled())
  with check (owner_user_id = (select auth.uid()) and public.ms_enabled());
drop policy if exists ms_sends_self on public.ms_sends;
...
drop policy if exists ms_suppr_select_self on public.ms_suppressions;
...
-- =====================================================================
-- >>> db/migrations/0019_bi_load_forecast.sql
-- =====================================================================
-- ============================================================================
...
-- =====================================================================
-- >>> db/migrations/0020_ms_assets_bucket.sql
-- =====================================================================
-- ============================================================================
