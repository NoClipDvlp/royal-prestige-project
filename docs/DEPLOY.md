...
| 9 | `db/migrations/0008_templates.sql` | Plantillas: `task_templates` + `template_items` + `template_assignments` + ALTER `tasks` (template_id/template_item_id/customized_at) + RLS admin-only. Aditiva → **después** de 0006 (ADR-0015). _(0007 = hueco intencional.)_ |
...
| 12 | `db/seed/roles.sql` | Intencionalmente **sin INSERTs** (no hay categorías de fábrica ni admin hardcodeado). Puede omitirse; se incluye por completitud. |
